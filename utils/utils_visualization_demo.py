        ...
        def onclick(event):
            if event.inaxes == axes[0]:
                with torch.no_grad():
                    ...
                    print(f"cos_map shape: {cos_map.shape}")
                    del trg_ft
                    gc.collect()
                    torch.cuda.empty_cache()
...
class DemoSingleImage(DemoInteractive):
    
    
    def __init__(self, imgs, ft, img_size, dist='argmax'):
        super(DemoSingleImage, self).__init__(imgs, ft, img_size, dist)

    
    def plot_img_pairs(self, query_coord, fig_size=3, alpha=0.45, scatter_size=70):
        
        assert len(query_coord) == 2, "query_coord should be a tuple of (x, y)"
        
        x, y = np.round(query_coord).astype(int)
